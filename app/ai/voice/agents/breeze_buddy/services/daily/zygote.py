...
Opt-in via BB_DAILY_BOT_ZYGOTE (default False). A failure before the payload
reaches the zygote returns None and the caller falls back to spawning; after
it, AmbiguousLaunch is raised instead, because a bot may already be running
and a second one would join the same room.

Needs UVICORN_RELOAD=false: the reloader runs the server in a subprocess that
imports this module fresh and cannot reach the socket.
"""
